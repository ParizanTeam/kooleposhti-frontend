...
export default class Day extends React.PureComponent {
  ...
  render() {
    return (
      ...
      >
        <div id="dayHeader" className={this.props.current?'':'inactiveDay'}>
          <div >
            <span className={this.isCurrentDate()?'currentDay':''}>{this.props.date.getDate()}</span>
          </div>
        </div>
